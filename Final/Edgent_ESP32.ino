...
void Light_sensor() {
  ...
  Serial.print("\t");
  Blynk.virtualWrite(V7, val);  // V7 is for light intensity
  Serial.print("\t");
}

void Soil_sensor() {
  double ini_soil_per = analogRead(34); // 類比輸入ADC
  if (isnan(ini_soil_per)) { // 如果沒有讀到ini_soil_per數值
  ...
  Blynk.virtualWrite(V8, soil_per);  // V8 is for soil sensor
...
BLYNK_WRITE(V1) { // control by button V1
  ...
  } else { // when press button send 0 to bs1
...
BLYNK_WRITE(V2) { // control by button V2
  ...
  } else { // when press button send 0 to bs2
